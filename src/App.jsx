...
export default function App() {
 ...
 const swapPerson = async (taskId, currentPerson, taskTitle) => {if (currentPerson === "Ambos") return;const newPerson = currentPerson === "Silvia" ? "Xavi" : "Silvia";const confirmed = window.confirm(`¿Cambiar "${taskTitle}" de ${currentPerson} a ${newPerson}?`);if (!confirmed) return;
                                                                 await setDoc(doc(db, "tasks", taskId), { person: newPerson }, { merge: true });};
  ...
  return (
    <div style={{minHeight:"100vh",background:"#f0f4f8",fontFamily:"'Nunito','Segoe UI',sans-serif",paddingBottom:60}}>
      <div style={{background:"linear-gradient(140deg,#1a2e52 0%,#2563eb 100%)",padding:"26px 20px 22px",borderRadius:"0 0 30px 30px",boxShadow:"0 6px 32px #1a2e5250"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            ...
            <button onClick={() => setShowAdd(true)} style={{background:"white",color:"#2563eb",border:"none",borderRadius:14,padding:"10px 18px",fontWeight:900,fontSize:20,cursor:"pointer",boxShadow:"0 2px 12px #0003",marginLeft:8}}>＋</button>
          </div>
        </div>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:"#ffffff18",borderRadius:16,padding:"10px 16px",marginBottom:16}}>
          <button onClick={() => prevSem && setSemana(prevSem)} style={{background:"none",border:"none",color:prevSem?"white":"#ffffff30",fontSize:24,cursor:prevSem?"pointer":"default",padding:"0 4px"}}>‹</button>
          <div style={{textAlign:"center"}}>
          ...
          <button onClick={() => nextSem && setSemana(nextSem)} style={{background:"none",border:"none",color:nextSem?"white":"#ffffff30",fontSize:24,cursor:nextSem?"pointer":"default",padding:"0 4px"}}>›</button>
        </div>
        <div style={{marginBottom:14}}>
          <div style={{display:"flex",justifyContent:"space-between",color:"#93c5fd",fontSize:12,marginBottom:5}}>
            <span>Progreso semanal</span><span style={{fontWeight:700}}>{doneSem}/{allSemTasks.length} ✓</span>
          </div>
          <div style={{background:"#ffffff25",borderRadius:99,height:11,overflow:"hidden"}}>
            <div style={{width:`${progSem}%`,height:"100%",background:"linear-gradient(90deg,#60a5fa,#34d399)",borderRadius:99,transition:"width 0.5s ease"}}/>
          </div>
          <div style={{color:"white",fontWeight:900,fontSize:17,textAlign:"right",marginTop:2}}>{progSem}%</div>
        </div>
        <div style={{display:"flex",gap:10}}>
          {PEOPLE.map(p => {
            const s = personStats(p); const ps = PERSON_STYLE[p]; const active = filterPerson === p;
            return (
              <div key={p} onClick={() => setFilterPerson(active ? null : p)} style={{flex:1,background:active?"white":"#ffffff18",borderRadius:18,padding:"10px 14px",display:"flex",alignItems:"center",gap:10,cursor:"pointer",border:`2px solid ${active?"white":"transparent"}`,transition:"all 0.2s"}}>
                <Ring pct={s.pct} color={active?ps.accent:"white"} size={46}/>
                <div>
                  <div style={{color:active?ps.accent:"white",fontWeight:800,fontSize:15}}>{ps.emoji} {p}</div>
      ...
      <div style={{padding:"18px 16px 0"}}>
        {Object.keys(byCategory).length === 0 && <div style={{textAlign:"center",padding:48,color:"#94a3b8",fontSize:15}}>🎉 ¡Sin tareas aquí!</div>}
          ...
          return (
            <div key={cat} style={{marginBottom:18}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:9}}>
                ...
                <span style={{background:allDone?"#dcfce7":"#f1f5f9",color:allDone?"#16a34a":"#94a3b8",borderRadius:99,padding:"2px 9px",fontSize:11,fontWeight:700}}>{catDone}/{catTasks.length}</span>
              </div>
              {catTasks.map(t => {
                const ps = t.person==="Ambos"?{bg:"#f3e8ff",accent:"#9333ea",emoji:"👫"}:PERSON_STYLE[t.person];
                return (
                  <div key={t.id} style={{background:"white",borderRadius:16,padding:"13px 14px",marginBottom:8,display:"flex",alignItems:"center",gap:12,boxShadow:"0 1px 8px #0000000a",border:`1.5px solid ${t.done?"#e2e8f0":ps.accent+"28"}`,opacity:t.done?0.55:1,transition:"all 0.2s"}}>
                    ...
                    <span onClick={() => swapPerson(t.id, t.person, t.title)} style={{background:ps.bg,color:ps.accent,borderRadius:10,padding:"3px 11px",fontSize:12,fontWeight:800,flexShrink:0,cursor:"pointer"}}>{ps.emoji} {t.person}</span>
        ...
        <div style={{background:"white",borderRadius:22,boxShadow:"0 2px 16px #0000000d",overflow:"hidden",marginTop:8,marginBottom:20,border:"1.5px solid #f0f4f8"}}>
          <div style={{background:"linear-gradient(120deg,#f59e0b,#f97316)",padding:"14px 18px 12px",display:"flex",alignItems:"center",gap:10}}>
            <span style={{fontSize:22}}>⭐</span>
            <div>
              <div style={{color:"white",fontWeight:900,fontSize:16}}>Extras de la semana</div>
              <div style={{color:"#fef3c7",fontSize:11}}>Tareas de más que habéis hecho · {semExtras.length} anotadas</div>
            </div>
          </div>
          <div style={{display:"flex",borderBottom:"1.5px solid #f1f5f9"}}>
            {PEOPLE.map(p => {
              const ps = PERSON_STYLE[p]; const count = semExtrasByPerson(p).length; const active = extraTab === p;
              return (
                <button key={p} onClick={() => setExtraTab(p)} style={{flex:1,padding:"11px 8px",border:"none",background:active?ps.bg:"white",color:active?ps.accent:"#94a3b8",fontWeight:active?800:600,fontSize:14,cursor:"pointer",transition:"all 0.2s",borderBottom:active?`3px solid ${ps.accent}`:"3px solid transparent"}}>
                  {ps.emoji} {p}
                  {count > 0 && <span style={{background:ps.accent,color:"white",borderRadius:99,padding:"1px 7px",fontSize:10,fontWeight:800,marginLeft:6}}>{count}</span>}
                </button>
              );
            })}
          </div>
          {(() => {
            const ps = PERSON_STYLE[extraTab]; const myExtras = semExtrasByPerson(extraTab);
            return (
              <div style={{padding:"14px 16px 16px"}}>
                <div style={{display:"flex",gap:8,marginBottom:14}}>
                  <input value={extraInput[extraTab]} onChange={e => setExtraInput({...extraInput,[extraTab]:e.target.value})} onKeyDown={e => e.key==="Enter" && addExtra(extraTab)} placeholder={`${extraTab}, ¿qué has hecho de más?`} style={{flex:1,padding:"10px 14px",borderRadius:12,border:`1.5px solid ${ps.accent}40`,fontSize:13,color:"#0f172a",outline:"none",fontFamily:"inherit",background:"#fafafa"}}/>
                  <button onClick={() => addExtra(extraTab)} style={{background:ps.accent,color:"white",border:"none",borderRadius:12,padding:"10px 16px",fontWeight:800,fontSize:16,cursor:"pointer",flexShrink:0}}>＋</button>
                </div>
                {myExtras.length === 0 ? <div style={{textAlign:"center",padding:"18px 10px",color:"#cbd5e1",fontSize:13}}>Nada anotado aún · escribe arriba ↑</div>
                : myExtras.map(e => (
                  <div key={e.id} style={{display:"flex",alignItems:"center",gap:10,background:ps.bg,borderRadius:12,padding:"10px 12px",marginBottom:7,border:`1px solid ${ps.accent}20`}}>
                    <span style={{fontSize:16,flexShrink:0}}>⭐</span>
                    <div style={{flex:1}}>
                      <div style={{fontWeight:700,color:"#0f172a",fontSize:13}}>{e.text}</div>
                      <div style={{color:ps.accent,fontSize:11,marginTop:1}}>{ps.emoji} {e.person} · {e.time}</div>
                    </div>
                    <button onClick={() => deleteExtra(e.id)} style={{background:"none",border:"none",color:"#cbd5e1",fontSize:13,cursor:"pointer",padding:2,flexShrink:0}}>✕</button>
                  </div>
                ))}
                {myExtras.length > 0 && <div style={{textAlign:"center",marginTop:10,background:`${ps.accent}12`,borderRadius:10,padding:"7px 10px",color:ps.accent,fontWeight:700,fontSize:12}}>🏆 {extraTab} ha hecho {myExtras.length} tarea{myExtras.length>1?"s":""} extra</div>}
              </div>
            );
          })()}
        </div>

        <div style={{background:"white",borderRadius:20,padding:"16px 18px",boxShadow:"0 1px 8px #0000000a",marginBottom:8}}>
          <div style={{fontWeight:800,color:"#1a2e52",fontSize:13,marginBottom:12}}>⚖️ Reparto esta semana</div>
          <div style={{display:"flex",gap:10}}>
            {PEOPLE.map(p => {
              const mine = allSemTasks.filter(t => t.person === p); const ps = PERSON_STYLE[p]; const xExtras = semExtrasByPerson(p).length;
              return (
                <div key={p} style={{flex:1,background:ps.bg,borderRadius:14,padding:"12px 10px",textAlign:"center"}}>
                  ...
                  <div style={{color:ps.accent,fontWeight:700,fontSize:12}}>{mine.filter(t=>t.done).length} completadas</div>
                  {xExtras > 0 && <div style={{background:ps.accent,color:"white",borderRadius:8,padding:"2px 8px",fontSize:11,fontWeight:800,marginTop:6,display:"inline-block"}}>⭐ {xExtras} extra{xExtras>1?"s":""}</div>}
                </div>
              );
            })}
          </div>
          <div style={{textAlign:"center",color:"#94a3b8",fontSize:11,marginTop:10}}>🔄 Rotación automática · 🔥 Sincronizado en tiempo real</div>
        </div>
      </div>

      {confetti && (
        <div onClick={() => setConfetti(false)} style={{position:"fixed",inset:0,background:"#00000065",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100}}>
          <div style={{background:"white",borderRadius:28,padding:"38px 44px",textAlign:"center",boxShadow:"0 8px 40px #0005"}}>
      ...
      {showAdd && (
        <div style={{position:"fixed",inset:0,background:"#00000055",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:50}}>
          <div style={{background:"white",borderRadius:"28px 28px 0 0",padding:"28px 22px 48px",width:"100%",maxWidth:500,boxShadow:"0 -4px 30px #0003"}}>
            <div style={{fontWeight:900,fontSize:20,color:"#1a2e52",marginBottom:18}}>➕ Añadir tarea</div>
            <label style={L}>Nombre</label>
            <input value={form.title} onChange={e => setForm({...form,title:e.target.value})} placeholder="Ej: Limpiar nevera" style={I} autoFocus/>
            ...
            <div style={{display:"flex",gap:10,marginTop:16}}>
              <button onClick={() => setShowAdd(false)} style={{flex:1,padding:14,borderRadius:14,border:"2px solid #e2e8f0",background:"white",color:"#64748b",fontWeight:700,cursor:"pointer",fontSize:15}}>Cancelar</button>
              <button onClick={addTask} style={{flex:2,padding:14,borderRadius:14,border:"none",background:"linear-gradient(120deg,#1a2e52,#2563eb)",color:"white",fontWeight:800,cursor:"pointer",fontSize:15}}>Añadir</button>
            </div>
          </div>
        </div>
      )}
